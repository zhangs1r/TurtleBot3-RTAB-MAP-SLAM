...
- `launch/turtlebot3_rtabslam.launch.py`：顶层入口，只负责编排与开关（是否启动传感器、RViz、rtabmap_viz、建图/定位模式）。
...
- `config/turtlebot3_rtabslam.rviz`：RViz 视图，固定 frame 为 `map`，并预置 `/scan`、`/rtabmap/map`、相机图像等显示。
...
ros2 launch turtlebot3_RTABSLAM turtlebot3_rtabslam.launch.py localization:=false
```
